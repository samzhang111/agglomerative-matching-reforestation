...
# 1a) Indigenous (i) vs. private lands (priv)
com_i_priv <- all_lands %>% 
  filter(land_type2 == "ti" | land_type2=="privadas") %>% 
  # create new binary variable so that MatchIt knows which is treatment and control variable
  mutate(treat = recode(land_type2, "ti"=1, "privadas"=0)) %>% 
  drop_na(area_final, forest1985, temp, precip, elevation, slope, pop_dens,
          dist_roads, dist_rivers, dist_cities) 

# 1b) Quilombola (q) vs. private lands (priv)
com_q_priv <- all_lands %>% 
  filter(land_type2 == "quilombola" | land_type2=="privadas") %>% 
  # create new binary variable so that MatchIt knows which is treatment and control variable
  mutate(treat = recode(land_type2, "quilombola"=1, "privadas"=0)) %>% 
  drop_na(area_final, forest1985, temp, precip, elevation, slope, pop_dens,
          dist_roads, dist_rivers, dist_cities) 
...
write_normalized <- function(df, fn) {
  df_used <- df %>% as.data.frame() %>%
    select(land_type2, treat, area_final, forest1985, temp, precip, elevation, slope,
           ...
           forest2004, forest2022, pasture1985, pasture2004, pasture2022, forest1985_perc,
           ephemeral_rest, persistent_rest, id)
    
  df_used_normalized <- df_used %>%
    mutate_at(vars(-land_type2, -treat, -ephemeral_rest, -persistent_rest, -id), ~ as.vector(scale(.x, center=FALSE, scale=TRUE)))
...
dfs <- capture_names(com_iq_priv, com_i_priv, com_q_priv, com_iq_pub, com_iq_pa, com_as_priv, com_as_pub, com_as_pa, com_pub_priv, com_pa_priv, com_pub_pa)
